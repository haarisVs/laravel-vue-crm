...
/* Delete item */
const deleteItem = () => {
  companiesStore.deleteItem(currentId.value)
}

const items = computed(() => companiesStore.data)
...
<template>
  ...
  <table>
    ...
    <tr>
      ...
      <th
        :class="['sortable uppercase text-sm font-normal text-pavitra-600', props.sortTitle === 'website' && props.sortDirection]"
        @click="sort('website')"
      >Website</th>

      <th
        :class="['sortable uppercase text-sm font-normal text-pavitra-600', props.sortTitle === 'logo' && props.sortDirection]"
        @click="sort('logo')"
      >Logo</th>
    ...
    <tr
      ...
      :key="client.id"
      @click="$router.push(`/companies/${client.id}/edit`)"
    >
      <TableCheckboxCell
        v-if="checkable"
